...
ให้ทำบท 18 ให้จบก่อน และตรวจว่า `AppDbContext` build ผ่าน:

```powershell
dotnet build
```

บทนี้ต้องมี SQL Server local ที่เชื่อมต่อได้อย่างน้อยหนึ่งแบบ เช่น SQL Server LocalDB บน Windows, SQL Server Developer Edition หรือ SQL Server container ผ่าน Docker
...
## เลือก SQL Server สำหรับ local development

หนังสือเล่มนี้ใช้ SQL Server เป็น database หลัก คุณเลือกได้สองทางที่พบบ่อย:

| ตัวเลือก | เหมาะกับใคร | จุดสำคัญ |
| --- | --- | --- |
| SQL Server LocalDB | Windows และ Visual Studio/.NET dev machine | ใช้ Windows authentication ไม่ต้องจำรหัสผ่าน `sa` |
| SQL Server container | เครื่องที่มี Docker | คุม environment ได้ชัด แต่ต้องตั้ง password ให้ container |

ถ้าเครื่องคุณมี LocalDB อยู่แล้ว ใช้ connection string แบบนี้ได้:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=(localdb)\\MSSQLLocalDB;Database=BackendApiDb;Trusted_Connection=True;TrustServerCertificate=True;"
  }
}
```

`Trusted_Connection=True` หมายถึงใช้ Windows account ปัจจุบันเชื่อมต่อ database จึงไม่ต้องใช้ `User Id=sa` หรือ password
...
ถ้าคุณใช้ LocalDB ให้ใช้ connection string แบบ LocalDB แทนตัวอย่าง Docker ด้านบน

ค่า password ในตัวอย่างเป็น placeholder สำหรับ local development เท่านั้น ห้ามใช้เป็น production secret และไม่ควร commit password จริงของทีมลง repository
