...
- `HTTP Request` คือคำขอที่ client ส่งมาหา API เช่น `GET /api/users`
- `HTTP Response` คือคำตอบที่ API ส่งกลับไป เช่น status code และ JSON
- `Endpoint` คือ URL และ HTTP method ที่ API เปิดให้เรียกใช้งาน เช่น `POST /api/auth/login`
...
- แอปธนาคารอาจมี `GET /api/accounts`, `GET /api/transactions` และ `POST /api/transfers`
- frontend อาจเรียก `GET /api/users/me` พร้อมส่ง `Authorization: Bearer <token>`
