...
- Diagram ด้วย Mermaid สำหรับ flow สำคัญ: เพิ่มรอบแรกแล้ว
- Traceability ของภาค 9 ไปยัง final example และ tests: เพิ่มรอบแรกแล้ว
...
- เพิ่ม Mermaid support ในเว็บหนังสือ และเพิ่ม diagram สำหรับ flow สำคัญ เช่น Controller/Service/Repository, DI, migration, exception handling, JWT, current user, authorization, audit log, integration test, Docker Compose, refresh token, email/reset password, CORS/rate limit/security headers และ health checks
- เพิ่ม implementation map ในบท 51-58 เพื่อบอกว่าแต่ละ production hardening feature อยู่ไฟล์ไหนใน `examples/final-backend-api` และตรวจด้วย test ไฟล์ใด พร้อม pseudo-code สำหรับ refresh token rotation/session revoke, email verification/reset password, middleware order และ incident triage
- เพิ่ม `public/favicon.svg` หลัง Release QA พบว่า preview เรียก `/favicon.svg` แล้วได้ `404`
- ตรวจ visual QA ด้วย Playwright fallback บน preview URL แล้ว: หน้าแรก, ภาค 9, บท 52, บท 54, บท 55 mobile และ search interaction ผ่านโดยไม่มี console/network error
...
Release QA ล่าสุดผ่านแล้ว:

- `SITE=https://example.github.io BASE_PATH=/aspnet-core-web-api-tutorial-book npm run build`
- Playwright preview smoke test ที่ `http://127.0.0.1:4323/aspnet-core-web-api-tutorial-book/`
- `dotnet test -c Release` สำหรับ final project ผ่าน 46 tests
- `dotnet test -c Release` สำหรับ validation/progressive project ผ่าน 51 tests
- `dotnet publish -c Release` สำหรับ final และ validation/progressive project ผ่าน
- `docker compose config --quiet` สำหรับ final และ validation/progressive project ผ่าน
- ตรวจ secret hygiene แล้ว ไม่พบ `.env` จริงที่ถูก track และไม่พบ pattern secret เสี่ยง
- Docker runtime smoke test ของ final project ผ่านแล้ว: `docker compose build`, `docker compose up -d`, `/health/live`, `/health/ready`, login ด้วย `admin@example.com / Passw0rd!`, `GET /api/admin/users`, `GET /api/auth/sessions` แล้วปิดและลบ volume ทดสอบด้วย `docker compose down -v`
...
- บท 51-58 เพิ่มตาราง `Feature / Files / Tests` เพื่อเชื่อมเนื้อหาหนังสือกับ implementation จริงใน final project และลดช่องว่างระหว่างการอ่านกับการเปิด code ตาม
