...
## Implementation map ใน repo นี้

| Feature | Files | Tests |
| --- | --- | --- |
| Secret placeholder และ local compose config | `examples/final-backend-api/.env.example`<br/>`examples/final-backend-api/docker-compose.yml`<br/>`.gitignore` | `examples/final-backend-api/README.md` |
| Runtime option validation ก่อน start | `examples/final-backend-api/Backend.Api/Program.cs`<br/>`examples/final-backend-api/Backend.Api/Options/JwtOptions.cs`<br/>`examples/final-backend-api/Backend.Api/Options/EmailOptions.cs` | `examples/final-backend-api/Backend.Api.Tests/ProductionHardeningIntegrationTests.cs` |
| Quality gates ของ final backend example | `examples/final-backend-api/Backend.Api.slnx`<br/>`examples/final-backend-api/Backend.Api/Dockerfile` | `examples/final-backend-api/Backend.Api.Tests/AuthIntegrationTests.cs`<br/>`examples/final-backend-api/Backend.Api.Tests/DatabaseModelTests.cs` |
| GitHub Pages deploy ของตัวหนังสือ | `.github/workflows/deploy.yml`<br/>`astro.config.mjs` | `package.json` |

ตารางนี้ชี้ไฟล์ที่มีอยู่จริงใน repo ส่วน YAML `backend-api` ด้านล่างเป็น template สำหรับ pipeline ของ backend API ถ้าแยก backend ไป deploy เป็น service จริง
...
jobs:
  test:
    ...
    steps:
      ...
      - run: dotnet restore examples/final-backend-api/Backend.Api.slnx
      - run: dotnet test examples/final-backend-api/Backend.Api.slnx --no-restore
      - run: docker build -t backend-api-ci -f examples/final-backend-api/Backend.Api/Dockerfile examples/final-backend-api
```
