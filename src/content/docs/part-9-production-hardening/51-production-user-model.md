...
บทนี้จะอธิบาย design, migration risk, ลำดับการแก้ code และจุดตรวจสำคัญไว้ในเนื้อหาเอง

ในขั้นนี้เราจะปรับ `User` ให้มี field สำคัญเพิ่ม เช่น `NormalizedEmail`, `IsEmailVerified`, `AccessFailedCount`, `LockoutEnd`, `LastLoginAt`, `PasswordChangedAt`, `RowVersion` และ relation ไปยัง `RefreshTokens`

จุดสำคัญคือ model ในภาคนี้ไม่ใช่การคัดลอกทับ `User` จากบท 17 แบบตรง ๆ เพราะบทก่อนหน้านี้ยังใช้ `int Id`, `CreatedAtUtc` และ `UpdatedAtUtc` อยู่ ส่วนภาคนี้จะยกระดับเป็น `Guid Id`, `DateTimeOffset` และชื่อ field แบบ production เช่น `CreatedAt` การเปลี่ยนนี้ต้องทำพร้อม repository, DTO, controller, current user claim, audit log และ migration ไม่เช่นนั้น project จะ compile ไม่ผ่าน
...
เมื่อทำครบตามลำดับนี้แล้ว ให้รัน test ทั้งหมดก่อนอ่านบทถัดไป เพราะบท refresh token จะต่อยอดจาก `Guid Id`, timestamp ใหม่ และ normalized email
...
นอกจาก unique index ของ `NormalizedEmail` แล้ว ควรเพิ่ม index สำหรับ query ที่ระบบใช้บ่อย:
...
Production migration ไม่ควรแค่เพิ่ม column แล้วสร้าง unique index ทันที เพราะถ้ามีข้อมูลเดิมอยู่ migration อาจพัง จึงต้องเติม `NormalizedEmail` จาก `Email` ก่อน แล้วค่อยบังคับ not null และสร้าง unique index
