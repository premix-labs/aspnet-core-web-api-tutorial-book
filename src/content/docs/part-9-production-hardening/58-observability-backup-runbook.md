...
## Pseudo-code สำหรับ incident triage

```text
HandleIncident(report):
  collect X-Correlation-Id, timestamp, endpoint, and user id if available
  search structured logs by correlation id
  check /health/live and /health/ready
  inspect recent audit logs for auth, token, admin, and password events
  compare error rate, latency, and database connection metrics

  if migration or deploy caused the issue:
    stop new rollout
    rollback image or restore database according to the runbook

  if secret or token leakage is suspected:
    rotate affected secret
    revoke affected refresh token sessions
    write an incident note with root cause and follow-up actions
```
