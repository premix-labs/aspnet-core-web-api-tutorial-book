...
บทนี้จะยัง search จาก `Email` โดยตรง เพราะ `NormalizedEmail` จะถูกเพิ่มตอน production user model ในบท 51

## คำศัพท์ในบทนี้

`Pagination` คือการแบ่งข้อมูลเป็นหน้า เช่นหน้า 1 แสดง 20 รายการ เพื่อไม่ให้ API ส่งข้อมูลทั้งหมดในครั้งเดียว

`Filtering` คือการกรองข้อมูล เช่นดูเฉพาะ role `Admin` หรือเฉพาะ user ที่ active

`Sorting` คือการเรียงข้อมูล เช่นเรียงตาม email หรือวันที่สร้าง
