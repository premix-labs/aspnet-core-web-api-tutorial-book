...
บทนี้จะปรับ `GET /api/v1/admin/users` ให้รองรับ pagination, filtering และ sorting
...
- `GET /api/v1/admin/users` คืนรายการผู้ใช้ได้
...
หลังจบบทนี้ response ของ `GET /api/v1/admin/users` จะเปลี่ยนจาก array ตรง ๆ เป็น object ที่มี `items`, `page`, `pageSize`, `totalItems` และ `totalPages`

## Query ที่ต้องรองรับ

```text
GET /api/v1/admin/users?page=1&pageSize=20&search=admin&role=Admin&isActive=true&sortBy=createdAtUtc&sortDirection=desc
```
...
ถ้าใช้ HTTPS ให้เปลี่ยน `baseUrl` เป็น port จริงของเครื่องคุณ เช่น `https://localhost:<https-port>`
...
- มี `PagedResponse<T>`
- มี `AdminUserQuery`
- `GET /api/v1/admin/users` รับ query string ผ่าน `[FromQuery]`
