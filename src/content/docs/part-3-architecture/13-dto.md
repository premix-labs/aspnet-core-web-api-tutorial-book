...
สร้างโฟลเดอร์ Dtos/Users
  ...
  -> ลบ record เดิมออกจาก Controller
...
ไฟล์นี้ใช้กับ `POST /api/users` เท่านั้น ความหมายคือ client ต้องส่ง email เข้ามาเพื่อสร้าง user
...
ไฟล์นี้ใช้กับ `PUT /api/users/{id}` แม้ตอนนี้ field เหมือน `CreateUserRequest` แต่แยกไว้เพราะ rule ตอนสร้างกับตอนแก้อาจต่างกันในอนาคต
...
บรรทัดนี้ทำให้ Controller มองเห็น `CreateUserRequest`, `UpdateUserRequest` และ `UserResponse` ที่ย้ายไปอยู่ใน namespace `Backend.Api.Dtos.Users`
...
ถ้าใน Controller ของคุณไม่มี `UserDto` แล้ว ไม่ต้องทำอะไรกับ `UserDto` ให้ลบเฉพาะ record ที่ยังมีอยู่จริง

ถ้าไม่ลบ `CreateUserRequest` หรือ `UpdateUserRequest` เดิม จะมีชื่อ DTO ซ้ำหรือทำให้ code สับสน
...
ถ้าเจอ error ว่า `CreateUserRequest` หรือ `UpdateUserRequest` ซ้ำ ให้ตรวจว่ายังมี record เดิมประกาศอยู่ใน `UsersController.cs` หรือไม่
