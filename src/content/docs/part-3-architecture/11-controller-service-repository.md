...
บทนี้เป็นบท concept ก่อนลงมือ refactor จริง ให้เน้นเข้าใจหน้าที่ของแต่ละชั้น ไม่ต้องสร้างไฟล์ทั้งหมดทันที

ถ้าอ่านแล้วรู้ว่า logic แบบไหนควรอยู่ใน Controller, Service หรือ Repository แปลว่าคุณพร้อมเข้าบท 12 แล้ว

## ก่อนเริ่มบทนี้

ให้ยืนยันว่าคุณทำภาค 2 จบแล้ว และ endpoint CRUD เดิมยังทำงาน:

```powershell
dotnet build
```

ในบทนี้ยังไม่ต้องแก้ code ถ้า build ไม่ผ่าน ให้แก้โปรเจกต์จากภาค 2 ก่อน เพราะบท 12 จะเริ่มย้าย code จาก `UsersController` ออกไปหลายไฟล์

## หลังจบบทนี้ ไฟล์ที่เปลี่ยน

บทนี้ยังไม่เปลี่ยนไฟล์ในโปรเจกต์ เป็นบทวางภาพรวมก่อน refactor จริง:

```text
ไม่มีไฟล์ code ที่ต้องแก้ในบทนี้
```

ไฟล์ที่จะเริ่มสร้างจริงจะอยู่ในบท 12
...
เมื่อ client เรียก `GET /api/v1/users/1` flow จะเป็นแบบนี้
