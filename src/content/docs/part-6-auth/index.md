...
ถ้าโปรเจกต์ของคุณใช้ versioned route กับ users เช่น `/api/v1/users` ให้เปลี่ยนเฉพาะ `@usersPath` เป็น `/api/v1/users`

`AuthController` ในหนังสือใช้ route คงที่ `/api/auth` จึงให้ `@authPath` เป็น `/api/auth` ตลอดภาคนี้
