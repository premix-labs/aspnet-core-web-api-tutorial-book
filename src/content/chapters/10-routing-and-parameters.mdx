---
title: "Routing และ Route Parameter"
description: "เข้าใจการจับคู่ URL กับ action method ใน Controller"
chapter: 10
icon: server
part: "ภาค 2: REST API"
difficulty: beginner
prerequisites: ["ทำ GET, POST, PUT, DELETE"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ Routing และ Route Parameter ได้"]
...
ให้ใช้ `UsersController` ที่ทำเสร็จจากบท 7 เป็นฐาน และตรวจว่า build ผ่านก่อน:

```powershell
dotnet build
```

บทนี้ไม่ได้บังคับให้เพิ่ม endpoint ใหม่ เป้าหมายหลักคืออ่าน route ที่มีอยู่ให้เข้าใจและแก้ปัญหา endpoint ไม่ถูกเรียกได้เอง

## หลังจบบทนี้ ไฟล์ที่เปลี่ยน

ถ้าคุณอ่านตามอย่างเดียว จะไม่มีไฟล์ที่ต้องเปลี่ยน

ถ้าคุณทดลอง query string ในหัวข้อกลางบท คุณจะแก้เฉพาะไฟล์นี้:

```text
Controllers/UsersController.cs
```

เมื่อทดลองเสร็จ ให้รัน `dotnet build` และตรวจว่า `GET /api/v1/users` แบบไม่ใส่ query string ยังทำงานได้ เพราะบทถัดไปจะใช้ endpoint เดิมเป็นฐาน
