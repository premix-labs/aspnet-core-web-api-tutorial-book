---
title: "ทดสอบ API ด้วย REST Client หรือ Postman"
description: "ทดสอบ endpoint ด้วยเครื่องมือภายนอกเพื่อดู request และ response จริง"
chapter: 12
icon: server
part: "ภาค 2: REST API"
difficulty: beginner
prerequisites: ["Request, Response และ Status Code"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ ทดสอบ API ด้วย REST Client หรือ Postman ได้"]
...
ให้ตรวจว่า code จากบท 7 ยัง build ผ่าน:

```powershell
dotnet build
```

จากนั้นรัน API แล้วใช้ port ที่ terminal แสดงจริงในไฟล์ `.http`

## หลังจบบทนี้ ไฟล์ที่เปลี่ยน

บทนี้แก้หรือสร้างไฟล์ request สำหรับทดสอบ API:

```text
Backend.Api.http
```

ไฟล์นี้ไม่ใช่ C# code แต่เป็นชุด request ที่ช่วยให้คุณกดทดสอบ endpoint เดิมซ้ำได้โดยไม่ต้องพิมพ์ URL ใหม่ทุกครั้ง
