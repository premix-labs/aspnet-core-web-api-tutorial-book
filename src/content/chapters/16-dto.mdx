---
title: "DTO คืออะไรและใช้เมื่อไหร่"
...
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ DTO คืออะไรและใช้เมื่อไหร่ ได้"]
...
ตรวจ `UsersController.cs` แบบเร็ว:

```text
ควรมี    using Backend.Api.Dtos.Users;
ควรมี    CreateUser(CreateUserRequest request)
ควรมี    UpdateUser(int id, UpdateUserRequest request)
ไม่ควรมี public record CreateUserRequest(...)
ไม่ควรมี public record UpdateUserRequest(...)
ไม่ควรมี public record UserDto(...)
```

ตรวจโฟลเดอร์ DTO:

```text
Dtos/Users/CreateUserRequest.cs  ใช้กับ POST /api/v1/users
Dtos/Users/UpdateUserRequest.cs  ใช้กับ PUT /api/v1/users/{id}
Dtos/Users/UserResponse.cs       เตรียมไว้สำหรับ response ในบท 14
```
