...
difficulty: beginner
prerequisites: ["แยก Controller, Service, Repository"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ Dependency Injection ได้"]
...
## Final shape checklist ของบทนี้

ใช้ checklist นี้ตรวจไฟล์หลัง refactor ก่อนรัน API ถ้าข้อใดไม่ตรง ให้กลับไปแก้ไฟล์นั้นก่อนทดสอบ endpoint

`Program.cs` ควรมี using และ DI registration:
...
`UsersController.cs` ควรมีลักษณะนี้:

```text
UsersController(IUserService userService)
  GetUsers()        เรียก userService.GetUsers()
  GetUserById(id)   เรียก userService.GetUserById(id)
  CreateUser(...)   เรียก userService.CreateUser(request.Email)
  UpdateUser(...)   เรียก userService.UpdateUser(id, request.Email)
  DeleteUser(id)    เรียก userService.DeleteUser(id)
```

`UsersController.cs` ไม่ควรมีสิ่งเหล่านี้แล้ว:
...
ถ้ายังเห็น code เหล่านี้ใน Controller แปลว่ายังย้าย logic ไป Repository ไม่ครบ

`InMemoryUserRepository.cs` เป็นที่เดียวในบทนี้ที่ควรมี in-memory list:
...
`UserService.cs` ควรเป็นตัวกลางระหว่าง Controller กับ Repository:

```text
UserService(IUserRepository userRepository)
GetUsers()       -> userRepository.GetAll()
GetUserById(id)  -> userRepository.GetById(id)
CreateUser(...)  -> userRepository.Add(...)
