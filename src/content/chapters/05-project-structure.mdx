---
title: "เข้าใจโครงสร้างไฟล์ของ ASP.NET Core"
description: "อธิบายไฟล์สำคัญใน ASP.NET Core Web API project"
chapter: 5
icon: settings
part: "ภาค 1: พื้นฐาน"
difficulty: beginner
prerequisites: ["สร้างโปรเจกต์แรก"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ เข้าใจโครงสร้างไฟล์ของ ASP.NET Core ได้"]
...
ให้เปิดโฟลเดอร์โปรเจกต์ที่สร้างในบทก่อน:

Windows PowerShell:

```powershell
cd "$HOME\code\aspnet-core-web-api-learning\Backend.Api"
```

macOS/Linux Bash:

```bash
cd ~/code/aspnet-core-web-api-learning/Backend.Api
```

ถ้าคุณเก็บงานไว้ path อื่น ให้ใช้ path จริงของเครื่องคุณ

## หลังจบบทนี้จะเปลี่ยนไฟล์อะไร

บทนี้ยังไม่ตั้งใจให้แก้ไฟล์ จุดประสงค์คืออ่านโครงสร้างและรู้ว่าเมื่อบทต่อไปบอกให้แก้ไฟล์ใด คุณจะหาไฟล์นั้นเจอ
...
หลัง build อาจมี `bin/` และ `obj/` เพิ่มเข้ามา:
...
สองโฟลเดอร์นี้เป็น output/generated files ของ .NET ไม่ใช่ source code ที่ต้องแก้เอง และโดยทั่วไปจะถูก ignore ใน Git
...
ถ้าเห็น warning เรื่อง package vulnerability ตอน build ให้ดูไฟล์ `.csproj` เพื่อรู้ว่ามี package อะไรบ้าง แต่ยังไม่ควรแก้ version แบบเดาสุ่ม ให้รอขั้นตอนที่หนังสือสอนจัดการ dependency หรืออ่าน advisory ก่อน
