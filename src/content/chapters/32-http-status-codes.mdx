---
title: "เลือกใช้ HTTP Status Code ให้ถูกต้อง"
description: "สรุปแนวทางเลือก status code สำหรับ validation, CRUD, auth และ business error"
chapter: 32
icon: shield-alert
part: "ภาค 5: Validation & Errors"
difficulty: intermediate
prerequisites: ["Error Response Format"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ เลือกใช้ HTTP Status Code ให้ถูกต้อง ได้"]
...
`traceId` จะเป็นค่าจริงที่ ASP.NET Core สร้างให้แต่ละ request จึงไม่จำเป็นต้องตรงกับตัวอย่าง
...
ถ้า client ขอข้อมูลด้วย id ที่ไม่มีอยู่จริง ให้ใช้ `404 Not Found`
...
| --- | --- | --- |
| Validation failed | `400` | `code = VALIDATION_FAILED`, `traceId` และ `errors.Email` |
| User not found | `404` | `code = USER_NOT_FOUND` และ `traceId` |
| Email conflict | `409` | `code = EMAIL_ALREADY_EXISTS` และ `traceId` |
