...
prerequisites: ["Global Exception Handler"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ Error Response Format ได้"]
...
- `traceId` id สำหรับเทียบ response กับ log ฝั่ง server
...
ควรได้ `409 Conflict` พร้อม `code` เป็น `EMAIL_ALREADY_EXISTS` และมี `traceId`
