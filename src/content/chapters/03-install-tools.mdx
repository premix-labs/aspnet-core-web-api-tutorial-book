---
title: "ติดตั้งเครื่องมือที่จำเป็น"
description: "เตรียมเครื่องมือสำหรับพัฒนา ASP.NET Core Web API บนเครื่อง local"
chapter: 3
icon: settings
part: "ภาค 1: พื้นฐาน"
difficulty: beginner
prerequisites: ["ASP.NET Core Web API คืออะไร"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ ติดตั้งเครื่องมือที่จำเป็น ได้"]
...
- Docker Desktop สำหรับรัน SQL Server และทดสอบ deployment ในบทหลัง
- SQL Server ผ่าน Docker container สำหรับภาคฐานข้อมูลในบทหลัง

สำหรับ Part 1 สิ่งที่ต้องพร้อมจริง ๆ คือ `.NET SDK`, editor และ terminal ส่วน Git, REST Client/Postman และ Docker จะเริ่มสำคัญขึ้นเมื่อเราทดสอบ API, เก็บ code และเข้าสู่ฐานข้อมูล

ถ้า Docker ยังไม่พร้อมในวันนี้ คุณยังสร้างและรันโปรเจกต์แรกได้ แต่อย่าลืมกลับมาแก้ก่อนเข้าสู่บทฐานข้อมูล เพราะตอนนั้นเราจะต้องมี database ที่รันได้จริง
...
Docker ยังไม่ใช่ blocker ของบทสร้างโปรเจกต์แรก ถ้า `docker version` ยังไม่ผ่าน ให้จดปัญหาไว้แล้วเรียนบท 3 ต่อได้ แต่ต้องแก้ให้ผ่านก่อนเริ่มภาคฐานข้อมูล
...
```text
<workspace>\aspnet-core-web-api-learning
```
...
3. ตรวจว่า Docker Desktop เปิดใช้งานได้หรือไม่ หรือจดไว้ว่าเครื่องนี้ต้องกลับมาแก้ Docker ก่อนภาคฐานข้อมูล
4. เปิด editor ที่จะใช้ตลอดเล่ม แล้วสร้างโฟลเดอร์สำหรับเรียน

## แนวคำตอบโดยย่อ

เครื่องพร้อมสร้างโปรเจกต์แรกเมื่อคำสั่งหลักเหล่านี้ทำงานได้:
...
เครื่องพร้อมเรียนทุกภาคของหนังสือเมื่อคำสั่งเหล่านี้ทำงานได้ด้วย:
...
ถ้า Docker ยังไม่ผ่าน คุณยังเรียนบทสร้างโปรเจกต์แรกได้ แต่ปัญหาจะกลับมาในบทฐานข้อมูลและ Docker ให้แก้ก่อนถึงจุดนั้น

## Checkpoint

ก่อนอ่านบทต่อไป ให้ตรวจสอบว่าคำสั่งหลักเหล่านี้ทำงานได้
...
ถ้าคำสั่งเหล่านี้ผ่าน คุณพร้อมสร้างโปรเจกต์แรกแล้ว

คำสั่ง Docker ควรกลับมาตรวจให้ผ่านก่อนภาคฐานข้อมูล:
...
ถ้า `docker version` ไม่ผ่านในตอนนี้ ให้จด error ไว้ก่อน บทถัดไปยังใช้เฉพาะ `dotnet` เป็นหลัก
