---
title: "Request, Response และ Status Code"
description: "เลือกชนิด response และ HTTP status code ให้เหมาะกับผลลัพธ์ของ API"
chapter: 11
icon: server
part: "ภาค 2: REST API"
difficulty: beginner
prerequisites: ["Routing และ Route Parameter"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ Request, Response และ Status Code ได้"]
...
คุณควรมี CRUD endpoint จากบท 7 แล้ว และควร build ผ่าน:

```powershell
dotnet build
```

ถ้า action ยังไม่ครบ ให้กลับไปทำบท 7 ก่อน เพราะบทนี้จะอธิบาย status code จาก action เหล่านั้น

## หลังจบบทนี้ ไฟล์ที่เปลี่ยน

บทนี้เป็นบทจัดความเข้าใจเรื่อง request, response และ status code จึงไม่บังคับให้แก้ไฟล์ใหม่

สิ่งที่ต้องทำคือเปิดไฟล์นี้ไว้เทียบกับบท:

```text
Controllers/UsersController.cs
```

แล้วตรวจว่า action แต่ละตัว return response helper ตรงกับตารางในบทนี้
...
เปิด `Controllers/UsersController.cs` แล้วตรวจแบบเร็ว:

```text
GetUsers()        -> return Ok(Users)
GetUserById(id)   -> return NotFound() หรือ Ok(user)
CreateUser(...)   -> return CreatedAtAction(...)
UpdateUser(...)   -> return NotFound() หรือ Ok(updatedUser)
DeleteUser(id)    -> return NotFound() หรือ NoContent()
```

ถ้าบรรทัด return ของคุณไม่ตรงกับ checklist นี้ ให้แก้ให้ตรงก่อนเข้าสู่บททดสอบ API
