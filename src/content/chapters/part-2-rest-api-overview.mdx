...
part: "ภาค 2: REST API"
difficulty: beginner
prerequisites: ["พื้นฐาน HTTP, REST และ JSON"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ ภาค 2 - Controller และ REST API ได้"]
...
คุณควรมีโปรเจกต์ `Backend.Api` จากภาค 1 และควรตรวจได้ว่าโปรเจกต์ยัง build ผ่าน:

```powershell
dotnet build
```

ถ้าเห็น warning แต่ยังขึ้น `Build succeeded.` และ `0 Error(s)` ให้เรียนต่อได้ก่อน ถ้ามี error ต้องแก้ก่อนสร้าง Controller เพราะ ASP.NET Core จะรัน endpoint ใหม่ไม่ได้ถ้า project compile ไม่ผ่าน
