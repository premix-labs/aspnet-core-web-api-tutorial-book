...
prerequisites: ["Response Format ที่อ่านง่ายและดูแลต่อได้"]
learningOutcomes: ["อธิบายและลงมือทำหัวข้อ ภาค 4 - เชื่อมต่อฐานข้อมูลด้วย EF Core ได้"]
...
- ไม่มี `Repositories/InMemoryUserRepository.cs` ค้างอยู่จนทำให้ `dotnet build` fail หลังเปลี่ยน repository interface เป็น async
