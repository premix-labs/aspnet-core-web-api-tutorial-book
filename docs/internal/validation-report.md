...
- 2026-07-22: นำ `astro-book-template` 1.2.2 มาใช้ครบทั้ง lifecycle metadata, managed UI, CI/CD, governance, book auditor, link checker, Playwright และ security gates; migrate frontmatter 74 หน้าเข้าสู่ schema กลาง, ปรับชื่อไฟล์ให้เลขตรงกับ `chapter`, เพิ่ม static redirect 58 เส้นทางเพื่อรักษา URL เดิม, ลบ H1 ซ้ำที่ layout สร้างให้อัตโนมัติ และสร้าง Windows visual baselines เฉพาะเล่ม ผล `npm run verify:enterprise` ผ่านครบ: Astro 0 errors/0 warnings, automated audit 0 findings, build 76 pages, link check ผ่าน, browser/accessibility 17 passed/1 skipped, visual 4 passed, performance 1 passed และ `npm audit` พบ 0 vulnerabilities; ตรวจ `template:check` แล้วติดตั้งเวอร์ชัน 1.2.2, ไม่มี update และ managed-file drift เป็น 0 ตัวอย่าง .NET ไม่ได้เปลี่ยน จึงไม่รัน `dotnet test` ใน migration นี้ ความเสี่ยงคงเหลือคือ `eslint-plugin-astro` ยังประกาศ engine เฉพาะ Node 22/24 จึงมี `EBADENGINE` warning บน Node 26 แม้ gate ทั้งหมดผ่าน แนะนำใช้ Node 24 ใน CI และงานเขียนตามปกติ
