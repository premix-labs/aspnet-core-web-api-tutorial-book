...
public class AuthIntegrationTests(TestApiFactory factory) : IClassFixture<TestApiFactory>
{
    ...
    [Fact]
    public async Task VerifyEmail_WhenTokenIsValid_MarksUserVerified()
    {
        var client = CreateClient();
        ...
        var verificationToken = GetVerificationToken("verify-success@example.com");
    ...
    public async Task Refresh_WhenTokenIsUsed_RotatesRefreshToken()
    {
        ...
        var storedTokens = await dbContext.RefreshTokens
            .AsNoTracking()
            .Where(token => token.UserId == loginBody.User.Id)
            .ToListAsync();
    ...
    public async Task AdminUsers_WhenSignedInAsRegularUser_ReturnsForbidden()
    {
        ...
        var registerResponse = await client.PostAsJsonAsync("/api/v1/auth/register", new
        {
            email = "regular-authz@example.com",
            password = "Passw0rd!"
        });
    ...
    [Fact]
    public async Task UsersEndpoint_WhenSignedInAsRegularUser_ReturnsForbidden()
    {
        var client = CreateClient();

        var registerResponse = await client.PostAsJsonAsync("/api/v1/auth/register", new
        {
            email = "regular-users-endpoint@example.com",
            password = "Passw0rd!"
        });
    ...
    [Fact]
    public async Task UsersEndpoint_WhenSignedInAsAdmin_ReturnsOk()
    {
        var client = CreateClient();

        var registerResponse = await client.PostAsJsonAsync("/api/v1/auth/register", new
        {
            email = "admin-users-endpoint@example.com",
            password = "Passw0rd!"
        });
